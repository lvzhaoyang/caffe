...
__global__ void PSROIPoolingForward(const int nthreads, const Dtype* bottom_data, 
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    // pick the right channel in bottom (permute the channels)
    ...
    // put the avg on top blob
    Dtype bin_area = static_cast<Dtype>(hend - hstart)*static_cast<Dtype>(wend - wstart);
...
void PSROIPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  PSROIPoolingForward<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >(
    ...
    pooled_height_, pooled_width_, bottom_rois, output_dim_, group_size_, 
    top_data, mapping_channel_ptr);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
__global__ void PSROIPoolingBackwardAtomic( const int nthreads, const Dtype* top_diff,
  const int* mapping_channel, const int num_rois,
  const Dtype spatial_scale, const Dtype spatial_offset,
  const int channels, const int height, const int width,
  const int pooled_height, const int pooled_width,
  const int output_dim, Dtype* bottom_diff, const Dtype* bottom_rois) {
  // There are two implementations for this function
  // (1) loop over top and using atomic add, which is bad for parallism
  // (2) loop over bottom and using many branches (ifs), which is bad on GPU
  // Anyhow, this is a cheap layer. Did not bother to opt ;)
...
void PSROIPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  PSROIPoolingBackwardAtomic<Dtype> << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >(
    count, top_diff, mapping_channel_ptr, top[0]->num(), spatial_scale_, spatial_offset_, 
