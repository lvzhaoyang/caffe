...
void SmoothL1LossLayer<Dtype>::LayerSetUp(
  ...
  if (has_weights_) {
    CHECK_EQ(bottom.size(), 3) << "If weights are used, must specify both "
      "inside and outside weights";
  }
  if (loss_param.pre_fixed_normalization()) {
    has_normalizer_ = true;
    normalizer_ = loss_param.normalizer();
  } else {
...
void SmoothL1LossLayer<Dtype>::Reshape(
  ...
  // reset normalizer if necessary
  if (!has_normalizer_) {
    normalizer_ = bottom[0]->num();
  }
