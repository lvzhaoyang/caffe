...
void WeightedSoftmaxWithLossLayer<Dtype>::Reshape(
  ...
  CHECK_EQ(outer_num_ * inner_num_, bottom[2]->count())
      ...
      << "e.g., if softmax axis == 1 and prediction shape is (N, C, H, W), "
      << "weights count (number of labels) must be N*H*W, "
...
void WeightedSoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const Dtype* weight = bottom[2]->cpu_data();
    int dim = prob_.count() / outer_num_;
    int channels = dim / inner_num_;
    int count = 0;
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        const int label_value = static_cast<int>(label[i * inner_num_ + j]);
        const Dtype weight_value = weight[i * inner_num_ + j];
        // special case (set gradients to 0)
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          // the gradient should be w_i (p_i - l_i)
          // = w_i * p_i | w_i * (p_i - 1)
          // rescale the gradients aross all channels of current sample
